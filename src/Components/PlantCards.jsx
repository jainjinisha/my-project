...
const PlantCard = ({img,name,desc,price }) => {
    ...
    <div className=" backdrop-blur-xl rounded-xl bg--gradient-to-r from white/20 via black/0 to-white/20">
        ...
        <img src={img} alt={name} className="w-[459px] h-[459px] object-contain"/>
        <h3 className="text-xl font-semibold text-white ">{name}</h3>
        <p className="text-white/80 mt-2 px-3">{desc}</p>
        <div className="flex items-center justify-betweem ">
            <span className="text-bold text-white">Rs. {price}/-</span>
            <button className="px-3 py-2 rounded-lg bg-white text-black text-sm"><FiShoppingBag size={20} /></button>
        </div>
      </div>
    </div>
    )
